...
class ContatoFirebase: ContatoDao {
    ...
    private val USER_DB = "users"
    // Referência para o nó principal que é a lista de contatos do RtDb
    private val contatosListRtDb = AutenticadorFirebase.firebaseAuth.currentUser?.let {
        Firebase.database.getReference(USER_DB).child(it.uid)
            .child(CONTATOS_LIST_REALTIME_DATABASE)
    }
